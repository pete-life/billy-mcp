...
import {Store,digest,type Batch,type BatchLease,type BatchStage} from './store.js';
...
const payment=z.strictObject({entryDate:date,cashAmount:positive,cashSide:z.enum(['debit','credit']),cashAccountId:identifier,bankLineId:identifier,subjectAmount:positive.optional(),subjectCurrencyId:identifier.optional(),cashExchangeRate:z.number().finite().positive().optional()});
...
export class BatchManager {
  ...
  private publicView(batch:Batch){const {runnerPid,leaseNonce,...visible}=batch;return visible;}
  ...
  private async stage(lease:BatchLease,key:string,op:unknown,reason:string){
    ...
    if(plan.status==='rejected'){
      ...
      if(refreshed.hash!==plan.hash)throw new Error(`Batch stage ${key} changed after rejection; refresh the unstarted batch or create a new batch for remaining cases and obtain new approval.`);
      plan=refreshed;
    }
