...
if(!c.token){console.log('Missing BILLY_ACCESS_TOKEN. Create a company API token in Billy and save it using billy-mcp setup. Do not paste it in chat.');process.exitCode=1;}
  ...
  try {
    ...
    console.log(JSON.stringify({connected:true,organization:{id:org.id,name:org.name,registrationNo:org.registrationNo},pinned:Boolean(c.organizationId),writesEnabled:c.writes,bankMatchingEnabled:c.bankMatching},null,2));
  }catch(e){console.error(e instanceof Error?e.message:'Connection failed');process.exitCode=1;}
}
